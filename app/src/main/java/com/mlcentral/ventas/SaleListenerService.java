...
public class SaleListenerService extends Service {
    ...
    private void listenLoop() {
        ...
        while (running) {
            ...
            try {
                ...
                SaleStore.setConnected(this, false); updateServiceNotification("Reconectando…");
    ...
    private void handleReadSync(JSONObject o) {
        ...
        SaleStore.markRead(this, ids); cancelSaleNotifications(ids); broadcastRefresh();
    }
    ...
    private void handleMessage(JSONObject o) {
        ...
        if (isDeliveryEvent(title)) {
            ...
            SaleStore.markDeliverySeen(this, shipmentId); showDeliveryNotification(shipmentId, title, message); broadcastRefresh(); return;
        }
        if (isSaleUpdate(title)) {
            SaleStore.markSeen(this, saleId); SaleStore.updateHistory(this, saleId, title, message, when); broadcastRefresh(); return;
        }
    ...
    private void broadcastRefresh() { sendBroadcast(new Intent("com.mlcentral.ventas.SALE_RECEIVED").setPackage(getPackageName())); }
    ...
    private void createChannels() {
        ...
        service.setDescription("Mantiene la conexión para recibir ventas nuevas."); service.setShowBadge(false); nm.createNotificationChannel(service);

        NotificationChannel sales = new NotificationChannel(SALES_CHANNEL, "Ventas nuevas", NotificationManager.IMPORTANCE_HIGH);
        sales.setDescription("Aviso inmediato cuando ML Central detecta una venta."); sales.enableVibration(true); sales.setVibrationPattern(new long[]{0, 350, 140, 350, 140, 700});
        ...
        sales.setSound(sound, attrs); sales.setLockscreenVisibility(Notification.VISIBILITY_PUBLIC); nm.createNotificationChannel(sales);
        ...
        delivered.setDescription("Aviso cuando Mercado Libre confirma que un paquete fue entregado."); delivered.enableVibration(true); delivered.setVibrationPattern(new long[]{0, 220, 120, 220}); delivered.setLockscreenVisibility(Notification.VISIBILITY_PUBLIC); nm.createNotificationChannel(delivered);
    }
    ...
    private Notification serviceNotification(String text) {
        Notification.Builder b = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O ? new Notification.Builder(this, SERVICE_CHANNEL) : new Notification.Builder(this);
        return b.setSmallIcon(android.R.drawable.ic_dialog_info).setContentTitle("ML Central Ventas activo").setContentText(text).setOngoing(true).setOnlyAlertOnce(true).setContentIntent(openAppIntent(7)).build();
    }
    ...
    private void showDeliveryNotification(String shipmentId, String title, String message) {
        Notification.Builder b = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O ? new Notification.Builder(this, DELIVERY_CHANNEL) : new Notification.Builder(this);
        b.setSmallIcon(android.R.drawable.stat_sys_download_done).setContentTitle(title).setContentText(firstLine(message)).setStyle(new Notification.BigTextStyle().bigText(message)).setContentIntent(openAppIntent(("delivery:" + shipmentId).hashCode())).setAutoCancel(true).setWhen(System.currentTimeMillis()).setShowWhen(true).setCategory(Notification.CATEGORY_STATUS).setVisibility(Notification.VISIBILITY_PUBLIC);
